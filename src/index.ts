...
const isUrl = /^(http|https):\/\//.test(userInput) && userInput.includes('pinterest.com');
...
if (isUrl) {
    ...
    targetUrl = `https://pinterest.com/search/pins/?q=${encodeURIComponent(userInput)}`;
}
...
const pinterest = new Pinterest(pinterestOptions.websiteURL, pinterestOptions.headless);
const webhook = new WebhookClient(process.env.WEBHOOK_URL as string, process.env.WEBHOOK_TOKEN as string);
